...
static int		get_color_px(double complex z[2], void (*fract)(double complex z[2]),
	int max_c)
{
	int				c;
	int				d;
...
static void		ft_put_pixel(t_data *data, int px[2])
{
	...
	z[0] = CMPLX(data->c_point.real, data->c_point.imag);
	z[1] = CMPLX(((px[0] / data->zoom) + data->ref_x),
((px[1] / data->zoom + data->ref_y)));
	px2[0] = px[0] + (W_W / 2);
	px2[1] = px[1] + (W_H / 2);
